...
/bin/rm -f ${T_DIR}/get_event_history.jtl ${T_DIR}/jmeter.log  > /dev/null 2>&1

./run.sh -Dlog_level.jmeter=DEBUG \
	-JTARGET_HOST=${TARGET_HOST} -JTARGET_PORT=${TARGET_PORT} \
	-JTARGET_PATH=${TARGET_PATH} -JTARGET_KEYWORD=${TARGET_KEYWORD} \
	-n -t ${T_DIR}/get_event_history.jmx -l ${T_DIR}/get_event_history.jtl -j ${T_DIR}/jmeter.log \
	-e -o ${R_DIR}
...
cat ${T_DIR}/get_event_history.jtl
